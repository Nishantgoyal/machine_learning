...
class FlipPredictor(object):
    def __init__(self, coins):
        ...
        self.coins = coins
        n = len(coins)
        self.probs = [1/n for _ in range(n)]

    def pheads(self):
        return sum(
            self.coins[index] * self.probs[index]
    ...
    def update(self, result):
        if result == "H":
            ...
            ptails = 1 - self.pheads()
            for index in range(len(self.probs)):
                self.probs[index] = (
                    (1 - self.coins[index]) * self.probs[index]
                ) / ptails
...
testcases = [
    (
        ...
        [
            0.4166666666666667, 0.432,
            0.42183098591549295, 0.43639398998330553
        ]  # -- output
    ),
    (([0.14, 0.32, 0.42, 0.81, 0.21], 'HHHTTTHHH'), [0.5255789473684211, 0.6512136991788505, 0.7295055220497553,
                                                     0.6187139453483192, 0.4823974597714815, 0.3895729901052968, 0.46081730193074644, 0.5444108434105802, 0.6297110187222278]),
    (([0.14, 0.32, 0.42, 0.81, 0.21], 'TTTHHHHHH'), [0.2907741935483871, 0.25157009005730924, 0.23136284577678012,
                                                     0.2766575695593804, 0.3296000585271367, 0.38957299010529806, 0.4608173019307465, 0.5444108434105804, 0.6297110187222278]),
    (([0.12, 0.45, 0.23, 0.99, 0.35, 0.36], 'THHTHTTH'), [0.28514285714285714, 0.3378256513026052, 0.380956725493104,
                                                          0.3518717367468537, 0.37500429586037076, 0.36528605387582497, 0.3555106542906013, 0.37479179323540324]),
